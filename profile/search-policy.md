...
Export the candidate-local eligibility registry before discovery. Use only cited entries that are active on the run date and match every populated scope dimension. Expired, superseded, and out-of-scope entries cannot decide eligibility. Current canonical-vacancy evidence takes precedence, and a registry conflict routes the role to human review rather than automatically rejecting it.

Monitor `Shortlisted` and `Moved to Applications` canonical listings weekly. Compare listing status, location, work model, description hash, compensation publication/text, and eligibility. Persist the latest snapshot in `Lead Monitor` and material history in `Scan Log`; do not regress applications beyond the preparation stage.
