...
Live delivery remains separately disabled even after an outbox request exists. Local config stays at version 6; no connector endpoint, native destination target, or credential reference is added to `.job-search.local.json`. Instead, create a private profile under `state/notification-connectors/<profile_id>.profile.json` using the version-1 or version-2 connector-profile schema and values supplied by the real connector account. Do not invent or copy an endpoint or account identifier from documentation. Both versions support one bounded transport: HTTPS JSON with a bearer value read from a named process environment variable only at send time.

Version 1 preserves the original adapter-neutral JSON request body. Version 2 adds an exact rendering policy to each allowed destination: `adapter_neutral_json_v1` retains that behavior, while `slack_blocks_v1` renders the approved digest as bounded Slack Block Kit JSON. A Slack target is required only in the ignored private profile. It is never exported directly; the sanitized version-2 binding stores only its SHA-256 hash. Changing the renderer or target therefore invalidates the prior binding and requires a new `NCON-…` approval.
