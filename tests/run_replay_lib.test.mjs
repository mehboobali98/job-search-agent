...
function run(id, query, score) {
  return {
    ...
    replay_context: { query_plan: [{ query_id: "Q-1", finder: "backend_finder", role_family: "Backend / Platform", source: "web", lane: "backend", query }], filters: { country: "Worldwide" }, config: { alert_threshold: 80 }, evidence: { profile_hash: "abc" } },
...
test("produces stable replay hashes and explains material run differences", () => {
  ...
  assert.equal(first.role_family, undefined);
  assert.equal(first.queries[0].role_family, "Backend / Platform");
