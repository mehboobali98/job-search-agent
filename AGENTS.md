...
- Notification delivery-health inspection is read-only. It may inspect only connector outbox requests, sanitized receipts, and redacted connector recovery markers. It never reads profiles, credentials, endpoints, request items into its report, or candidate artifacts; it never writes state, invokes a connector, retries a delivery, or replaces `npm run pending` recovery guidance.
...
- `scripts/inspect_notification_health.mjs` produces a deterministic sanitized health report from private outbox requests, receipts, and recovery markers. It is strictly read-only and performs no profile, credential, endpoint, network, retry, or write operation.
