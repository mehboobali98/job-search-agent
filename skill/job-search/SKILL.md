...
- Treat notifications as a separate, disabled-by-default boundary over updater-returned alerts only. Preview first. Only exact approved `--apply` may create private local or connector-outbox requests; repository code never invokes a connector, stores credentials, bypasses quiet hours, submits applications, or contacts recruiters.
