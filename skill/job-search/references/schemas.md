...
`schemas/notification-connector-profile.v2.schema.json` retains the version-1 transport and limits while adding one exact rendering policy per allowed destination. `adapter_neutral_json_v1` has no native target; `slack_blocks_v1` requires a private opaque Slack target. `schemas/notification-connector-binding.v2.schema.json` exports the renderer ID and target hash but never the target itself. Both versions consume the unchanged version-1 delivery request, and receipts remain version 1.
...
`schemas/notification-connector-status-profile.v1.schema.json` defines a separate private, disabled-by-default HTTPS JSON bearer profile for one-attempt provider-status reads. `schemas/notification-connector-status-binding.v1.schema.json` exports only deterministic hashes, opaque references, the exact destination allowlist, bounded request policy, and explicit probe/single-attempt safety requirements. `schemas/notification-connector-status-observation.v1.schema.json` stores only the exact request hash, sanitized binding ID, timestamps, one of `delivered|rejected|pending|unknown`, bounded HTTP status, and no-secret/no-response/no-delivery safety flags.

`schemas/notification-connector-capability-export.v1.schema.json` defines a strict private read-only input produced outside repository code. It contains no endpoint or credential, but its account reference, native targets, and target labels remain private. `schemas/notification-connector-capability-catalog.v1.schema.json` defines the deterministic `NCAPCAT-…`/`NCAP-…` sanitized output with only an opaque connection reference, source hash, deterministic target IDs/hashes, channels, supported operations/renderers, counts, and explicit no-network/no-secret/no-delivery safety flags. The catalog remains private and never authorizes delivery.
