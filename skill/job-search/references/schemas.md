...
The updater validates the envelope, timestamps, agent states, count relationships, Search Config limits, query-attempt attribution, scan events, candidate hashes, eligibility, and alert rules before mutation. It writes one `Query Metrics` row per attempt and returns deterministic funnel, per-query, per-source, and coverage diagnostics. Payloads created before query-attempt tracking remain accepted and receive aggregate-only diagnostics.
