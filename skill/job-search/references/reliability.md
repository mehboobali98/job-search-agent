...
Use `npm run connector-profile-plan -- --catalog <private-catalog> --target <exact-target-id> --destination <exact-configured-destination> --renderer <exact-renderer>` for a bounded read-only authoring plan. The exact-name regular catalog must remain in private discovery state; the configured destination, catalog connection/channel, exact target, delivery operation, and renderer must agree. The command neither upgrades old configuration nor reads a raw export, profile, endpoint, credential source/value, or native target. It writes no profile or state, issues no approval, uses no network, and never authorizes delivery. Keep real plans private.
