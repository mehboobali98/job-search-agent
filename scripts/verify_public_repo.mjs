...
const names = (staged ? git(["diff", "--cached", "--name-only", "--diff-filter=ACMR"]) : git(["ls-files"]))
...
for (const name of names) {
  ...
  const content = staged ? git(["show", ":" + name]) : git(["show", "HEAD:" + name]);
  const patterns = [
...
console.log(JSON.stringify({ verified: true, mode: staged ? "staged" : "tracked", files_checked: names.length }, null, 2));
