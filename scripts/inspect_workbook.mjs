...
if (!workbookArgument || !renderDir) throw new Error("Usage: node scripts/inspect_workbook.mjs <xlsx> <render-dir>");
const workbookPath = resolveXlsxWorkbookPath(workbookArgument);
...
try {
  ...
  try { await removeWorkbookInspection(workbookPath); } catch { /* Inspection output is already available on stdout. */ }
}
