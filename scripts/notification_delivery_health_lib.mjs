...
export const NOTIFICATION_DELIVERY_HEALTH_SCHEMA_VERSION = 1;
...
const EVIDENCE = new Set(["none", "receipt", "recovery_marker"]);
const ACTION = new Set(["none", "wait_until_not_before", "review_outbox", "run_pending_inspection"]);
const ARTIFACT_TYPE = new Set(["request", "receipt", "recovery_marker"]);
...
function requestHealth(request, receipt, marker, asOf, staleAfterHours, hasArtifactConflict) {
  let status;
...
export function buildNotificationDeliveryHealthReport({
  requests = [], receipts = [], recoveryMarkers = [], artifactIssues = [],
  ...
  if (requests.length + receipts.length + recoveryMarkers.length > MAX_NOTIFICATION_HEALTH_ARTIFACTS) {
  ...
  const healthRequests = [...requestMap.values()]
    .map((request) => requestHealth(
      request, receiptMap.get(request.request_id), markerMap.get(request.request_id), timestamp,
  ...
  for (const item of report.requests) {
    ...
    if (item.delivery_evidence === "receipt") {
      if (item.status !== "confirmed" || item.receipt_id === null || item.binding_id === null
        ...
        || !new Set(["none", "run_pending_inspection"]).has(item.recovery_action)) {
      ...
      if (!new Set(["confirmed", "rejected", "unknown"]).has(item.status) || item.binding_id === null
        || item.recovery_action !== "run_pending_inspection") {
      ...
      || item.recovery_required || !new Set(["deferred", "queued", "stale"]).has(item.status)
